...
| `Transport.py` | `Transport.cpp` | yes | `thicket-interop` (`run_multihop_inbound.sh`) | **Partially covered as of 2026-08-04.** A transport-enabled reference node now sits between the Python originator and our leaf, so this is the first exercise of a packet that reached us *through* a router rather than off the same wire: the path is learned from a relayed announce, the packet arrives with a non-zero hop count, decrypts correctly, and the proof we emit travels back across the relay and is validated by the reference — which also exercises reverse-path handling. The hop count is asserted (the leaf reports it, the reference pins it), so a run that arrived directly fails rather than passes. **Residual gap: we are only ever the leaf.** Our own forwarding path — this build acting *as* a transport node — is still untested, and that is precisely what the local patch (store init regardless of `enable_transport`) touches. |
