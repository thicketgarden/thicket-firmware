#include "password.h"
// THICKET: was `#include <Utilities/OS.h>`. Upstream lives inside the
...
bool password_protect(const char* path, const char* password,
    ...
    // THICKET: was RNG.rand() from the Arduino Crypto library. Uses the
    // stack's own CSPRNG instead, so the host build has a source too.
    RNS::Bytes salt_bytes = RNS::Cryptography::random(16);
    RNS::Bytes iv_bytes   = RNS::Cryptography::random(16);
    memcpy(salt, salt_bytes.data(), 16);
    memcpy(iv,   iv_bytes.data(),   16);
