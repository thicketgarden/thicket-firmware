...
// sides. Four requests over one Link, asserting the current behaviour of the
// stack:
...
//      decompressor assembles it and the page matches.
//   3. /page/mid.mu, a 16 KB page (deterministic, generated the same way on both
//      sides), auto_compress=True. It compresses to ~8 KB, so it arrives as a
//      MULTI-PACKET compressed Resource and still fits under the cap. Must
//      succeed: this is the larger-page case, the one that exercises the decoder
//      on a real block rather than a single-packet toy.
//   4. /page/big.mu, a page over the on-device size cap (RNS_BUNZIP_CAP, 24 KB),
...
static const char* PATH_GZIP  = "/page/gz.mu";     // compressed, small (single-part)
static const char* PATH_MID   = "/page/mid.mu";    // compressed, multi-packet, under the cap
...
static RNS::Bytes mid_page;   // larger page, generated byte-identically on both sides
...
enum Phase { PH_PLAIN = 0, PH_GZIP = 1, PH_MID = 2, PH_BIG = 3, PH_DONE = 4 };
...
static volatile bool mid_done = false, mid_ok = false;
...
static bool matches(const RNS::Bytes& encoded, const RNS::Bytes& target, bool& used_unwrap) {
	RNS::Bytes page = encoded, unwrapped;
	used_unwrap = false;
	if (msgpack_unwrap_bin(encoded, unwrapped)) { page = unwrapped; used_unwrap = true; }
	return page.size() == target.size() &&
	       memcmp(page.data(), target.data(), page.size()) == 0;
}

// A deterministic page over a 16-symbol alphabet: it compresses to about half
// its size, so it is large enough to arrive as a multi-packet compressed
// Resource yet decompresses under the cap. The same LCG runs on the Python side,
// so the bytes match exactly. Mirrors glibc rand(): full-width multiply, masked
// to 31 bits (done in 64-bit here so it matches Python's arbitrary precision).
static RNS::Bytes gen_page(size_t n) {
	RNS::Bytes out;
	uint32_t x = 0x1234u;
	static const char A[] = "0123456789ABCDEF";
	for (size_t i = 0; i < n; ++i) {
		x = (uint32_t)((((uint64_t)x * 1103515245ull) + 12345ull) & 0x7fffffffull);
		uint8_t b = (uint8_t)A[(x >> 16) & 0xF];
		out.append(&b, 1);
	}
	return out;
}
...
static void on_response(const RNS::RequestReceipt& receipt) {
	...
	bool uw = false;
	if (phase == PH_PLAIN) {
		plain_ok = matches(encoded, expected_page, uw);
		plain_done = true;
		printf("[cpp] /page/index.mu: %lu bytes, match=%s\n",
		       (unsigned long)encoded.size(), plain_ok ? "yes" : "no");
	} else if (phase == PH_GZIP) {
		gz_ok = matches(encoded, expected_page, uw);
		gz_unwrapped = uw;
		gz_done = true;
		printf("[cpp] /page/gz.mu: decompressed, match=%s (bz2 assembled on-device)\n",
		       gz_ok ? "yes" : "no");
	} else if (phase == PH_MID) {
		mid_ok = matches(encoded, mid_page, uw);
		mid_done = true;
		printf("[cpp] /page/mid.mu: %lu-byte page decompressed, match=%s (multi-packet compressed Resource)\n",
		       (unsigned long)mid_page.size(), mid_ok ? "yes" : "no");
...
static void on_failed(const RNS::RequestReceipt& receipt) {
	...
	else if (phase == PH_MID) { mid_done = true; printf("[cpp] /page/mid.mu FAILED (larger bz2 decompress did not complete)\n"); }
	else if (phase == PH_BIG) {
...
static void request_phase() {
	...
	const char* path = phase == PH_PLAIN ? PATH_PLAIN : phase == PH_GZIP ? PATH_GZIP : phase == PH_MID ? PATH_MID : PATH_BIG;
...
int main() {
	...
	mid_page = gen_page(16384);   // ~16 KB -> ~8 KB compressed, multi-packet, under the cap
	...
	while (true) {
		...
		if (now - start > TIMEOUT_S) {
			printf("[cpp] TIMEOUT (announce=%d link=%d plain=%d gz=%d big=%d)\n",
			       announce_seen, link_up, plain_done, gz_done, big_done);  // mid between gz and big
			break;
		}
	...
	printf("[cpp]   %s larger compressed page (multi-packet, %lu B) decompressed and matched\n", mid_ok ? "OK  " : "FAIL", (unsigned long)mid_page.size());
