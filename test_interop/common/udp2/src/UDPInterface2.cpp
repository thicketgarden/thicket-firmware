...
UDPInterface2::UDPInterface2(const char* name, int local_port, int remote_port,
	...
	// THICKET: the whole reason this file exists. See UDPInterface2.h.
...
/*virtual*/ UDPInterface2::~UDPInterface2() {
	stop();
}

//bool UDPInterface2::start(const char* wifi_ssid, const char* wifi_password, int port /*= DEFAULT_UDP_PORT*/, const char* local_host /*= nullptr*/) {
/*virtual*/ bool UDPInterface2::start() {
	...
	TRACEF("UDPInterface2: remote port: %d", _remote_port);
 
#ifdef ARDUINO
	TRACEF("UDPInterface2: wifi ssid: %s", _wifi_ssid.c_str());
	TRACEF("UDPInterface2: wifi password: %s", _wifi_password.c_str());
...
/*virtual*/ void UDPInterface2::stop() {
#ifdef ARDUINO
#else
	if (_socket > -1) {
		close(_socket);
...
/*virtual*/ void UDPInterface2::loop() {
...
/*virtual*/ bool UDPInterface2::send_outgoing(const Bytes& data) {
...
void UDPInterface2::on_incoming(const Bytes& data) {
